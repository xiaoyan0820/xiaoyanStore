        ...
        >
            ...
            <el-table-column prop="img" label="图片">
                <template slot-scope="item">
                    <!-- slot-scope="item" 绑定slot-scope要根据后端进行变更 -->
        ...
        subInfo(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    for (let i in data) {
                        file.append(i, data[i]) // append()是文件上传 i是key值 data[i]是value值
                    }
                    //单独对图片地址进行操作
