...
export default {
  ...
  methods: {
    ...
    getTime(e) {
      ...
      console.log(e);//包含开始时间戳和结束时间戳的对象
      console.log(this.seckInfo.begintime, this.seckInfo.endtime);
    },
    //封装一个获取秒杀活动列表事件
    ...
    update(id) {
      ...
      getseckInfo({ id }).then((res) => {
        ...
        if (res.data.code == 200) {
          console.log(res);
          this.seckInfo.begintime = this.dateValue[0].toString();
          this.seckInfo.endtime = this.dateValue[1].toString();
          console.log(this.seckInfo.begintime, this.seckInfo.endtime);
          ...
          console.log(this.secondArr);
          console.log(this.seckInfo.second_cateid);
          console.log(res.data.list.first_cateid);
        }
      });
    },
    //确定添加或者更新事件
