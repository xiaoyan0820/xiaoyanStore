    ...
    >
      <el-form :model="memberInfo" :rules="rules" ref="memberInfo">
        <!-- <el-form-item
                    ...
                    prop="roleid"
                >
                    <el-select v-model="memberInfo.roleid" placeholder="请选择">
                        <el-option
                            v-for="item in getStateMemberList"
                            :key="item.id"
                            :label="item.phone"
                            :value="item.id"
                        >{{item.phone}}</el-option>
                    </el-select>
        </el-form-item>-->
...
export default {
  ...
  computed: {
    ...mapGetters(["getStateMemberList", "getStateMemberList"]),
  },
  mounted() {
    //组件一加载就调取用户接口
    //触发才调取vuex中的用户列表
  ...
  //this.isAdd = false;
    ...
    subInfo(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          console.log(this.memberInfo, "会员表单");
          //根据isAdd状态去判断执行接口
        //   if (this.isAdd) {
        //     //调取添加接口
    ...
    //             if(this.pageInfo.page !=1 && this.getStateMemberList.length==1){
    //                 this.pageInfo.page--
    //             }
    //              //调取获取用户接口列表的行动
    //                 this.$store.dispatch('getActionMemberList', this.pageInfo)
    //         }
    //     })
    // },
    //当页面发生变化的时候触发该方法
